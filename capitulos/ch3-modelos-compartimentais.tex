...
compartimento com pessoas infectadas (I) e considerando uma população de tamanho $N$, 
obtem-se as seguintes definições:
...
\begin{equation}\label{eq:analise-de-sensibilidade-local}
...
Considerando que a variância $V_i$ pode ser calculada pela fórmula \ref{eq:variancia-metodo-sobol-1}.

\begin{equation}\label{eq:variancia-metodo-sobol-1}
    V_i = var_{x_i}(E_{x_{~i}}(Y|X_i))
\end{equation}

E que a variância $V_{ij}$ pode ser calculada pela fórmula \ref{eq:variancia-metodo-sobol-2}.

\begin{equation}\label{eq:variancia-metodo-sobol-2}
    V_{ij} = var_{x_{ij}}(E_{x_{~ij}}(Y|X_i,X_j)) - V_i - V_j
\end{equation}

Então a variância da saída $\mathcal{Y}$ é definida pela equação \ref{eq:variancia-sobol}.

\begin{equation}\label{eq:variancia-sobol}
    var(\mathcal{Y}) = \sum_{i=1}^{d}V_i + \sum_{i<j}^{d} V_{ij} + \dots +  V_{dd}
\end{equation}
...
Baseados no conjunto de modelos apresentados em \cite{kermack-mcKendrick:1927} modelo \textit{SIR}, 
foram propostos outros modelos com mais compartimentos, 

Como primeiro exemplo, pode-se mencionar o \textit{SIRD} (Susceptible-Infected-Recovered-Deacesed) 
...
a doença (D). Um novo parâmetro de taxa de mortalidade $\alpha$ é adicionado. 
...
Ao longo dos anos foram propostas diferentes variantes do módelo \textit{SIR} 
...
das mais diversas fontes. São utilizados dados de incidência de uma pandemia como 
número de hospitalizados, mas também são incorporados dados de pesquisas de redes 
sociais, criando uma framework que trabalha com dados de alta dimensionalidade.
